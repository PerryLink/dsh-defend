...
- Repair GBK mojibake in historical CHANGELOG entries: em/en dashes (U+9225 U+003F / U+9225 U+63F1), middle dots (U+8DEF, the mangled 8·14/8·16 postmortem dates) and an ellipsis (U+9225 U+E6C6) are restored to the clean pre-corruption text; no behavior change.
...
- Align the devDependency pins to the published dsh `0.1.2-alpha.3` line (8 `@deepseek-ai/dsh-*` packages) and align `cordis`/`schemastery` to `^4.0.2`/`^3.18.2`. The audit gate classification keeps failing safe on `0.1.2-alpha.3` (`Session.append` still cannot stamp the `ignorable` marker); the five-language READMEs record the alpha.3 fact.
