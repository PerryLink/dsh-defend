...
- `tests/` — vitest; real Cordis `Context` + real `SessionStore`/`Session`/`ToolRuntime`/`Commands`/`ApprovalService` from the `0.1.2-alpha.3` peers; the agent object is a structural fake. `guard-hook.spec.ts` covers the destructive-delete guard, `detect.spec.ts` the pure layer, `defend-detect.spec.ts` the interception wiring through real waterfalls, `audit-support.spec.ts` the ignorable-marker host-capability degradation.
...
`typecheck` resolves `@deepseek-ai/*` from the installed `0.1.2-alpha.3` devDependencies (this repo has no tsconfig paths); `typecheck:ci` checks against the published `0.1.2-alpha.3` types (explicit empty paths). Both stay green — see dsh-click's AGENTS.md for the shared rationale.
