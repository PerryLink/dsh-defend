...
| Surface | Status |
|---|---|
| Harness | DeepSeek Harness `0.1.1-rc.2` (peer ranges `>=0.1.0-rc.8 <0.2.0`; verified against checkout `0.1.2-alpha.2` on 2026-08-31) |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
| `detection.allowUnmarkedAudit` | `false` | Keep writing session audit on hosts whose `Session.append` predates the `ignorable` marker (every released line so far) or that fail-closed on unknown event types (host `0.1.2-alpha.2`+), accepting the unresumable-session hazard |
